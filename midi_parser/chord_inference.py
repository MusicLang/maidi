...
from .chord_inference_utils import TEMPLATES, COEFFS, CHROMA_VECTORS_MATRIX, PITCH_CLASSES_DICT, \
    CHORD_TYPE_TO_PITCHES, CHORD_TYPES, PITCHES_DICT, PITCHES_DICT_ROOT
...
def get_chroma_vectors(pitches_bars):
    ...
    value_counts = [np.bincount(pitches_class, minlength=12) for pitches_class in pitches_class_array]
...
def get_pitch_class_set_tonality(tonality_root, tonality_mode):
    ...
    pitch_classes = [(pitch_class + tonality_root) % 12 for pitch_class in pitch_classes]
...
def optimal_chord_inference(candidates, chord_durations, bass_notes=None, bars=None):
    ...
    chords = []
    for (roman, tonality_root, tonality_mode, extension), chord_duration, bass_note in zip(chords_props, chord_durations, bass_notes):
...
def get_octave_from_chord(roman, tonality_root, tonality_mode, extension):
    ...
    chord_pitches_real = PITCHES_DICT_ROOT[(roman, tonality_root, tonality_mode, real_extension)]
...
def get_chord_extended_from_bass_note(bass_note, chord):
    ...
    chord_pitches_real = PITCHES_DICT_ROOT[(roman, tonality_root, tonality_mode, extension)]
...
def fast_chord_inference(tracks, chord_durations):
    ...
    pitches_bars = [concatenate_pitches([track_bar[i]['pitch'] for (idx, program, is_drum), track_bar in tracks.items() if not is_drum])
    ...
    candidates = []
    for chord_roots, chord_types, chroma in zip(chord_rootss, chord_typess, bar_chroma_vectors):
        all_sub_candidates = []
