...
from midi_parser.constants import INSTRUMENTS_DICT, API_URL_VARIABLE, API_KEY_VARIABLE

class MusicLangScore:
    ...
    def from_base64(cls, base64_string, tpq=24):
        ...
        :param tpq: int, ticks per quarter note
        :return:
        """
    ...
    def delete_bar(self, bar_index):
        ...
        :param bar_index: int, index of the bar to delete
        :return:
        """
        self = self.copy()
        ...
        removed_chord_duration = removed_chord[self.TIME_TICK_END_INDEX] - removed_chord[self.TIME_TICK_START_INDEX]
        ...
        return MusicLangScore(new_chords, new_tracks, self.track_keys, self.tempo, self.tpq)
    ...
    def add_bar(self, bar_index):
        ...
        chord_duration = new_chords[bar_index][self.TIME_TICK_END_INDEX] - new_chords[bar_index][self.TIME_TICK_START_INDEX]
        # Offset the start and end time of the chords
        ...
        for track_key in self.track_keys:
            idx, program, is_drum, voice = track_key
            new_tracks[(idx, program, is_drum, voice)].insert(bar_index + 1, self.get_fake_note_vector())
    ...
    def write(self, filename):
        ...
        for chord in self.chords:
            ts = TimeSignature(chord[self.TIME_TICK_START_INDEX],
                               chord[self.TIME_SIGNATURE_NUMERATOR_INDEX],
                               chord[self.TIME_SIGNATURE_DENOMINATOR_INDEX])
        ...
        for track_key in self.track_keys:
            ...
            track.notes = self.track_notes_to_score_notes(self.chords, self.tracks[(idx, program, is_drum, voice)])
    ...
    def track_notes_to_score_notes(cls, chords, track_notes):
        ...
        for idx_bar, chord_track_notes in enumerate(track_notes):
            for idx in range(len(chord_track_notes['time'])):
                time = chord_track_notes['time'][idx] + chords[idx_bar][MusicLangScore.TIME_TICK_START_INDEX]
                ...
                has_note = True
        if not has_note:
            notes.append(Note(0, cls.FAKE_NOTE_DURATION, cls.FAKE_NOTE_PITCH, cls.FAKE_NOTE_VELOCITY))
        return notes
    ...
    def offset_notes(offset, tracks, track_keys):
        ...
        for track_key in track_keys:
            idx, program, is_drum, voice = track_key
            new_tracks[(idx, program, is_drum, voice)] = deepcopy(tracks[(idx, program, is_drum, voice)])
            for notes in new_tracks[(idx, program, is_drum, voice)]:
                notes['time'] = np.asarray(notes['time'].copy(), dtype=np.int32) + offset
                assert all([time >= 0 for time in notes['time']]), f'Negative time found in track {track_key}'
    ...
    def offset_score(self, offset):
        ...
        return MusicLangScore(new_chords, deepcopy(self.tracks), deepcopy(self.track_keys), self.tempo, self.tpq)
    ...
    def get_score_between(self, start_bar, end_bar):
        ...
        assert all([chord[self.TIME_TICK_START_INDEX] >= 0 for chord in new_chords]), f'Negative time found in chords {new_chords}'
        new_tracks = {}
        for track_key in self.track_keys:
            idx, program, is_drum, voice = track_key
            new_tracks[(idx, program, is_drum, voice)] = self.tracks[(idx, program, is_drum, voice)][start_bar:end_bar]
            assert len(new_tracks[(idx, program, is_drum, voice)]) == len(new_chords), 'Track and chord length mismatch'
        return MusicLangScore(new_chords, new_tracks, self.track_keys, self.tempo, self.tpq)

    def copy(self):
        return MusicLangScore(deepcopy(self.chords), deepcopy(self.tracks), deepcopy(self.track_keys), self.tempo, self.tpq)
    ...
    def get_empty_controls(self, prevent_silence=False):
        ...
        :param prevent_silence: bool, if True add a tag to each bar to explicitly prevent silence during generation
        :return:
        """
        mask = self.get_mask()
    ...
    def prevent_silence_tags(self, tags):
        ...
        :param tags: list of list of list of str, tags to prevent silence
        :return:
        """
    ...
    def get_track_subset(self, start, end):
        ...
        score.tracks = {track_key: score.tracks[track_key] for track_key in score.track_keys}
        return score

    def get_silenced_mask(self):
        """
        Return a mask that tells which bars are silent (no notes or fake notes)
        :return:
        """
        mask = np.zeros((len(self.track_keys), len(self.chords)))
        for i, track_key in enumerate(self.track_keys):
            ...
            for j, chord in enumerate(self.chords):
                mask[i, j] = self.tracks[(idx, program, is_drum, voice)][j]['pitch'].shape[0] == 0
        return mask
    ...
    def cut_silenced_bars_at_beginning_and_end(self):
        ...
        while silenced_bars[-bar_to_cut_end - 1]:
            ...
            if bar_to_cut_end == len(silenced_bars):
                break
        score = score.get_score_between(bar_to_cut_start, len(score.chords) - bar_to_cut_end)
        return score

    def check_mask(self, mask):
        """
        Check that the mask is valid
        :param mask: np.array of shape (n_tracks, n_chords)
        :return:
        """
        if mask.shape[1] != len(self.chords):
            raise ValueError(f'Mask length does not match score length {mask.shape[1]} != {len(self.chords)}')
        if mask.shape[1] > 16:
            raise ValueError('Mask length must be less than 16')
        if mask.shape[0] != len(self.track_keys):
            raise ValueError('Mask number of tracks does not match score number of tracks')
    ...
    def _remove_temp_midi_files(self, prompt_file, output_file, midi_file, output_midi_file):
    ...
    def _call_predict_api(self, midi_base64, mask, model, temperature, **prediction_kwargs):
    ...
    def _predict_with_api(self, mask, model='large', temperature=0.95, async_mode=False,
                          polling_interval=1,
                          **prediction_kwargs):
        mask = np.asarray(mask)
        self.check_mask(mask)
        task_id = self._call_predict_api(self.to_base64(), mask, model, temperature, **prediction_kwargs)
        if async_mode:
            return task_id
        else:
    ...
    def _predict_with_predictor(self, predictor, mask, model='large', timeout=3000, temperature=0.95, **prediction_kwargs):
        mask = np.asarray(mask)
        self.check_mask(mask)
        prompt_file, output_file, midi_file, output_midi_file = self._create_temp_midi_files()
        self.write(midi_file)
        predictor(midi_file, output_midi_file, prompt_file, output_file, mask,
                  chord_range=(0, mask.shape[1]), model=model,
                  timeout=timeout, temperature=temperature, **prediction_kwargs)
        score = MusicLangScore.from_midi(output_midi_file)
        # Clean up the files
        self._remove_temp_midi_files(prompt_file, output_file, midi_file, output_midi_file)
        return score

    def check_times(self):
        for track_key in self.track_keys:
            idx, program, is_drum, voice = track_key
            assert len(self.tracks[(idx, program, is_drum, voice)]) == len(self.chords), 'Track and chord length mismatch'
    ...
    def cut_notes_to_end_of_bars(self):
        ...
        for track_key in self.track_keys:
            idx, program, is_drum, voice = track_key
            new_tracks[(idx, program, is_drum, voice)] = deepcopy(self.tracks[(idx, program, is_drum, voice)])
            for idx_chord, chord in enumerate(self.chords):
                ...
                for idx_note, note in enumerate(new_tracks[(idx, program, is_drum, voice)][idx_chord]['time']):
                    if note + new_tracks[(idx, program, is_drum, voice)][idx_chord]['duration'][idx_note] > chord_duration:
                        new_tracks[(idx, program, is_drum, voice)][idx_chord]['duration'][idx_note] = chord_duration - note

        return MusicLangScore(deepcopy(self.chords), new_tracks, deepcopy(self.track_keys), self.tempo, self.tpq)
    ...
    @classmethod
    def from_task_id(cls, task_id, polling_interval=1):
        """
        Create a score from a task id
        :param task_id: str, task id
        :param timeout: int, timeout for the API
        :param polling_interval: int, interval in seconds to poll the API
        :return:
        """
        url, key = cls.get_env()
        output_midi_file = tempfile.NamedTemporaryFile(suffix='.mid').name
        ...
        while True:
            ...
            print("Still pending", result)
            time.sleep(polling_interval)

        print(result)
        # Download the file
        with open(output_midi_file, 'wb') as f:
            f.write(base64.b64decode(result['result']['midi']))
        score = cls.from_midi(output_midi_file)
        # Clean up the files
        os.remove(output_midi_file)

        return score

    def predict(self, mask, model='control_masking_large', timeout=3000,
                temperature=0.95, cut_silenced_bars=False, regen_missing_bars=False,
                async_mode=False,
                polling_interval=1,
                **prediction_kwargs):
        """
        ...
        :param mask: np.array of shape (n_tracks, n_chords)
        :param predictor: Predictor, function to call the API
        :param model: str, model to use (for the API)
        :param timeout: int, timeout for the API
        :param temperature: float, temperature for the model
        :param cut_silenced_bars: bool, if True cut silenced bars at the beginning and end
        :param regen_missing_bars: bool, if True regenerate missing bars with another call to predict
        :param prediction_kwargs: dict, additional arguments for the model (for example chord and control tags)
        :param async_mode: bool, if True return the task id, otherwise wait for the request to finish with polling
        :param polling_interval: int, interval in seconds to poll the API, only used if async_mode is False
        :return: MusicLangScore or str : predicted score if sync mode
        """

        score_to_predict = self.copy()
        mask = np.asarray(mask)
        score_to_predict.check_mask(mask)
        result = score_to_predict._predict_with_api(mask, model=model, timeout=timeout, temperature=temperature,
                                                   async_mode=async_mode, polling_interval=polling_interval,
                                                   **prediction_kwargs)
        if async_mode:
        ...
        if regen_missing_bars and score.nb_bars < mask.shape[1]:
            ...
            return score.predict(new_mask, model=model, timeout=timeout, temperature=temperature,
                                 cut_silenced_bars=cut_silenced_bars, regen_missing_bars=regen_missing_bars)
        return score
    ...
    def add_track(self, program, is_drum):
        ...
        :param is_drum: bool, is drum track
        :return:
        """
        self = self.copy()
        new_track_key = (len(self.track_keys), program, bool(is_drum), 0)
        self.track_keys.append(new_track_key)
        self.tracks[new_track_key] = [self.get_fake_note_vector() for _ in range(len(self.chords))]
        return self
    ...
    def match_track_keys(cls, score, other_score):
        ...
        :param other_score: MusicLangScore, Score on which to match track keys
        :return:
        """
        score = score.copy()
        ...
        for i, track_key in enumerate(shorter_score.track_keys):
            if track_key != longer_score.track_keys[i]:
                raise ValueError('Score A and B have totally different tracks, cannot match them')
        # Add fake tracks to the shorter score
        for i in range(len(shorter_score.track_keys), len(longer_score.track_keys)):
            shorter_score = shorter_score.add_track(longer_score.track_keys[i][1], longer_score.track_keys[i][2])
    ...
    def change_instrument(self, track_idx, instrument):
        ...
        print(len(score.track_keys))
        new_track_index = (track_key[0], INSTRUMENTS_DICT[instrument][0], bool(INSTRUMENTS_DICT[instrument][1]), track_key[3])
        score.track_keys[track_idx] = new_track_index
        # Change in tracks
        score.tracks[new_track_index] = score.tracks.pop(track_key)
        print(len(score.track_keys), len(score.tracks))
        return score
    ...
    def add(self, other_score, silence_bars=0, try_matching_tracks=True):
        ...
        :param other_score: MusicLangScore
        :param silence_bars: int, number of silence bars to add between the two scores
        :param try_matching_tracks: bool, if True try to match the tracks of the two scores
        :return:
        """
        score = self.copy()
        if silence_bars > 0:
            ...
            assert len(score.chords) == initial_length + silence_bars, 'Silence bars not added correctly'
        other_score = other_score.copy()
        ...
        for track_key in score.track_keys:
            idx, program, is_drum, voice = track_key
            new_tracks[(idx, program, is_drum, voice)] = score.tracks[(idx, program, is_drum, voice)] \
                                                         + new_other_score.tracks[(idx, program, is_drum, voice)]
        return MusicLangScore(new_chords, new_tracks, score.track_keys, score.tempo, score.tpq)

    def add_silence_bars(self, n_bars, add_fake_notes=True):
        """
        Add n_bars of silence to the end of the score
        :param n_bars: int, number of silence bars to add
        :param add_fake_notes: bool, if True add a fake note to each track
        :return:
        """
        score = self.copy()
        # Assert that the other score has the same tracks
        ...
        offset = score.chords[-1][score.TIME_TICK_END_INDEX]
        if n_bars <= 0:
            raise ValueError('n_bars must be positive')  # If we want to add silence bars
        offset += n_bars * (
                    score.chords[-1][score.TIME_TICK_END_INDEX] - score.chords[-1][score.TIME_TICK_START_INDEX])
        new_chords = score.chords
        time = new_chords[-1][score.TIME_TICK_END_INDEX]
        last_bar_duration = score.chords[-1][score.TIME_TICK_END_INDEX] - score.chords[-1][score.TIME_TICK_START_INDEX]
        ...
        for track_key in score.track_keys:
            ...
            for new_chord in new_chords[-n_bars:]:
                if add_fake_notes:
                    new_tracks[(idx, program, is_drum, voice)].append(self.get_fake_note_vector())
                else:
                    new_tracks[(idx, program, is_drum, voice)].append({'time': np.asarray([], dtype=np.int32), 'pitch': np.asarray([], dtype=np.int8),
                                                                       'duration': np.asarray([], dtype=np.int32), 'velocity': np.asarray([], dtype=np.int8)})
            # Assert good length
            assert len(new_tracks[(idx, program, is_drum, voice)]) == len(new_chords), 'Track and chord length mismatch'
        return MusicLangScore(new_chords, new_tracks, score.track_keys, score.tempo, score.tpq)
    ...
    def from_empty(cls, instruments, nb_bars, ts=(4, 4), tempo=120, tpq=24):
        ...
        track_keys = [(idx, INSTRUMENTS_DICT[ins][0], INSTRUMENTS_DICT[ins][1] == 1, 0) for idx, ins in enumerate(instruments)]
        chords = [[0, 0, 'M', 0, '', ts[0], ts[1],
